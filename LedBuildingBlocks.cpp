...
float BellWave::intensity_distance(float dist) {
    return h * intensity_coeff * exp(-dist * dist * h * h / sigma / sigma); //higher height, more concentrated h = 1/w
}

bool BellWave::kill_condition() {
    bool val = height() < kill_threshold && vh < 0 || this->location_raw < -right_max_radius || this->location_raw > led_count + left_max_radius;
    bool vala = height() < kill_threshold;
    bool valb = vh < 0;
    bool valc = this->location_raw < -right_max_radius;
    bool vald = this->location_raw > led_count + left_max_radius;
    // if (val) std::cout << "killing bellwave" << vala << valb << valc << vald << "\n";
    return val;
}
...
void BellWave::set_hvelocity(float vh) {
    this->vh = vh;
}
...
BellWave::BellWave(float vi, Colors::RGBu8 color, float intensity_coeff, float sigma, float x, float hi, float vh, int left_max, int right_max, int leds, float kill_threshold, float split_threshold) {
    this->v = vi;
    this->c = color;
    ...
    this->right_max_radius = right_max;
    this->led_count = leds;
    this->location_raw = this->location = x;
    if (left_max_radius >= led_count || right_max_radius >= led_count) {
        if (!NOTHROW) {
            throw "Left or right calculation cutoff radius on a LightPoint object cannot be greater than (or equal to) led count";
        } else {
...
void RandomBellWaves::entity_handler(float dt) {
    ...
    if (rand01 < waveProbability(dt, elapsed)) {
        ...
        // std::cout << "adding bellwave\n";
        add_bellwave(
            new BellWave(
                waveVelocity(elapsed), waveColor(elapsed), 1.0, sigma, waveX(elapsed), 0.01, waveHeightVelocity(elapsed), leftmax, rightmax, get_led_count(), min_intensity_threshold, max_intensity_threshold
            )
        );        
    }
    for (int i = 0; i < wave_entities.size(); i++) {
        ...
        if (auto wave = wave_entities[i].lock()) {
            if (wave->height() > max_intensity_threshold && wave->hvelocity() > 0) {
                wave->alter_intensity_coeff(0.5);
                auto waveCopy = new BellWave(*wave);
                float impulse = 1;//waveSplitVelocity(elapsed);
                wave->set_velocity(wave->velocity() + impulse);
                waveCopy->set_velocity(waveCopy->velocity() - impulse);
                wave->set_hvelocity(-wave->hvelocity());
                // std::cout << "split attempt\n";
                add_bellwave(waveCopy);
            }
        }
    }
}
